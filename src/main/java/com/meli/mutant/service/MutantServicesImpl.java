...
public class MutantServicesImpl implements IMutantService {
    ...
    public Boolean isMutant(Dna dna) {
        ...
        String dnaSec[][] = new String[dna.getDna().size()][dna.getDna().size()];
        ...
        if ( validateHorizontal(dnaSec) || validateVertical(dnaSec) || diagonal(dnaSec) || reverseDiagonal(dnaSec)) {
    ...
    private Boolean reverseDiagonal(String dna[][]) {
        String previousLetter = "";
        int height = dna.length;
        for (int v = 0; v < height; v++) {
            int sec = 1;
            if (v > 0) {
                for (int vertical = v, horizontal = height - 1;
                     vertical <= height && horizontal >= v;
                     vertical++, horizontal--) {
                    if (previousLetter.equals(dna[vertical][horizontal])) {
                        sec++;
                        if (sec == 4) {
                ...
                for (int diagonal = 0; diagonal < height; diagonal++) {
                    for (int vertical = v, horizontal = diagonal;
                         vertical <= diagonal && horizontal >= v;
                         vertical++, horizontal--) {
                        if (previousLetter.equals(dna[vertical][horizontal])) {
                            sec++;
                            if (sec == 4) {
